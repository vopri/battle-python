...
from battle_snake.interactor import FirstStep, MyFutureHistory, Tactics
...
def test_my_future_history_dead_step_1(solo_board_2):
    ...
    for _ in range(8):
        fb.next_turn()
    assert (
        hist.all_my_snakes_definitely_dead_after_how_many_steps(
            FirstStep(NextStep.RIGHT)
        )
        == 1
    )
...
def test_my_future_history_dead_lock(solo_board_4):
    ...
    for _ in range(8):
        fb.next_turn()
    assert (
        hist.all_my_snakes_definitely_dead_after_how_many_steps(FirstStep(NextStep.UP))
        == 3
    )
